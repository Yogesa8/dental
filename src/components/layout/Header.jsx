...
import dentalLogo from "../../assets/common/dental-logo.png";
...
const Header = () => {
  ...
  const headerVisible = hiddenOnPath !== pathname;
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        setHiddenOnPath(entry.isIntersecting ? null : pathname);
    ...
    >
      <nav className="mx-auto flex w-full max-w-4xl items-center justify-between gap-4 rounded-full border border-white/90 bg-white/60 px-3 py-2 shadow-sm backdrop-blur-xl sm:px-4 md:gap-6">
        <a
          ...
          aria-label="Mex Dental Care home"
        >
          <img
            src={dentalLogo}
            alt=""
          ...
          />
          <span className="text-xs sm:text-sm">MEXDENTALCARE</span>
        </a>
